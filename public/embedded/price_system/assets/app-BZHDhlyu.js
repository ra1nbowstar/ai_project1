...
function qe() {
    return `
        <form id="warehouse-form">
            ...
            <div class="form-group">
                <label for="warehouse-type-select">库房类型</label>
                <select id="warehouse-type-select" class="form-control">
            ...
            <div class="form-group">
                <label for="warehouse-contact">库房联系人</label>
                <input type="text" id="warehouse-contact" class="form-control" placeholder="可选">
            </div>
            <div class="form-group">
                <label for="warehouse-phone">电话</label>
                <input type="text" id="warehouse-phone" class="form-control" maxlength="20" inputmode="numeric" autocomplete="tel" placeholder="选填；填写时须 11 位数字（空格、横线可忽略）">
            </div>
            <div class="form-group">
                <label for="warehouse-hazmat-permit-qty">危废经营许可数量</label>
                <input type="number" id="warehouse-hazmat-permit-qty" class="form-control" step="1" min="0" placeholder="可选">
            </div>
            <div class="form-group">
                <label for="warehouse-monthly-inbound">月均收货</label>
...
function Je() {
    return `
        <form id="freight-form">
            ...
            <div class="form-group">
                <label for="freight-smelter">冶炼厂 <span class="required">*</span></label>
                <select id="freight-smelter" class="form-control" required>
                    <option value="">请选择冶炼厂</option>
                    <!-- 冶炼厂选项将通过JavaScript动态生成 -->
                </select>
            </div>
            <div class="form-group">
                <label for="freight-price">运费（元/吨） <span class="required">*</span></label>
...
function Ye() {
    return `
        <form id="freight-edit-form">
            ...
            <div class="form-group">
                <label for="freight-edit-price">运费（元/吨） <span class="required">*</span></label>
                <input type="number" id="freight-edit-price" class="form-control" step="0.01" min="0" required>
            </div>
            <div class="form-group">
                <label for="freight-edit-effective-date">生效日期</label>
                <input type="date" id="freight-edit-effective-date" class="form-control">
...
function Xe() {
    return `
        <form id="category-form">
            ...
            <div class="form-group">
                <label for="category-alias">别名</label>
                <input type="text" id="category-alias" class="form-control">
                <small class="form-text">多个别名用逗号分隔</small>
            </div>
            <div class="form-group">
                <label for="category-unit">计量单位</label>
    ...
    return `
        <form id="tax-config-form">
            <div class="form-group">
                ...
                <select id="tax-smelter" class="form-control" required>
                    <option value="">请选择冶炼厂</option>
                </select>
            </div>
            <div class="form-group">
                <label for="tax-type">计税口径 <span class="required">*</span></label>
                <select id="tax-type" class="form-control" required>
                    <option value="1pct">1%增值税</option>
                    <option value="3pct" selected>3%增值税</option>
                    <option value="13pct">13%增值税</option>
                </select>
                <small class="form-text">与接口 <code>tax_type</code> 一致；同一冶炼厂可按不同口径各配一条换算比例。</small>
            </div>
            <div class="form-group">
                <label for="tax-ratio">换算比例 <span class="required">*</span></label>
                <input type="number" id="tax-ratio" class="form-control" step="0.0001" min="0" placeholder="如 0.03 或填 3 表示 3%" required>
...
async function K() {
    ...
    if (n && n.value && (o.pageSize = parseInt(n.value, 10) || o.pageSize), e) {
        ...
        try {
            ...
            <td>${o||`-`}</td>
            <td>${c||`-`}</td>
            <td>${isNaN(u)?`-`:u.toLocaleString()}</td>
            <td>${d}</td>
            <td>
...
function renderModalWhCombo(e) {
    renderWhComboList(e, modalWhRows, {
        syncSearchOnPick: !0
    })
}
async function setupModalWhCombo(e) {
    var t = await Api.request(`GET`, `/tl/get_warehouses`);
    modalWhRows = Api.unwrapList(t).filter(function(e) {
        return !b(T(e))
    });
    var n = document.getElementById(e + `-warehouse`),
        r = document.getElementById(e + `-warehouse-search`);
    n && (n.value = ``), r && (r.value = ``), r && (r.oninput = function() {
        renderModalWhCombo(e)
    }, r.onkeydown = function(t) {
        if (t.key === `Enter`) {
            t.preventDefault();
            var n = document.getElementById(e + `-warehouse-list`);
            if (!n) return;
            var r = n.querySelector(`button[data-wh-id]`);
            r && r.click()
        }
    }), renderModalWhCombo(e)
}
async function invFillCategorySelect(e) {
    var t = document.getElementById(e);
    if (!t) return;
    var n = await Api.request(`GET`, `/tl/get_category_mapping`),
        r = V(Api.unwrapData(n) || {}).list,
        i = ue(se(Array.isArray(r) ? r : []));
    t.innerHTML = `<option value="">请选择品种</option>` + i.map(function(e) {
        var t = e.id,
            n = String(e.name || ``).trim();
        return t != null && !isNaN(Number(t)) && n ? `<option value="` + String(t) + `">` + M(n) + `</option>` : ``
    }).join(``)
}

function invUpdatePager() {
    var e = document.getElementById(`inv-page-info`),
        t = document.getElementById(`inv-prev-btn`),
        n = document.getElementById(`inv-next-btn`),
        r = Math.max(1, Math.ceil((invPg.total || 0) / invPg.pageSize));
    e && (e.textContent = `第 ` + invPg.page + ` / ` + r + ` 页（共 ` + invPg.total + ` 条）`), t && (t.disabled = invPg.page <= 1), n && (n.disabled = invPg.page >= r)
}
async function invManualSubmit() {
    var e = parseInt(document.getElementById(`inv-warehouse`).value, 10),
        t = parseInt(document.getElementById(`inv-category`).value, 10),
        n = parseFloat(document.getElementById(`inv-stock`).value),
        r = document.getElementById(`inv-date`),
        i = r && r.value ? r.value.trim() : ``;
    if (!e || isNaN(e)) throw Error(`请选择库房`);
    if (!t || isNaN(t)) throw Error(`请选择回收品种`);
    if (isNaN(n) || n < 0) throw Error(`请填写有效库存`);
    var a = {
        库房id: e,
        品类id: t,
        当前库存: n
    };
    i && (a.库存日期 = i), await Api.request(`POST`, `/tl/warehouse_inventories`, a), invPg.page = 1, await loadInventoryList()
}
async function invDownloadTemplate() {
    try {
        var e = await Api.getBlob(`/tl/download_warehouse_inventory_template_excel`),
            t = window.URL.createObjectURL(e),
            n = document.createElement(`a`);
        n.href = t, n.download = `库房库存导入模板.xlsx`, document.body.appendChild(n), n.click(), n.remove(), window.URL.revokeObjectURL(t)
    } catch (e) {
        alert(`下载模板失败: ` + (e.message || String(e)))
    }
}
async function invImportExcel(e) {
    var t = e && e[0];
    if (!t) return;
    if (!/\.(xlsx|xls)$/i.test(String(t.name || ``))) {
        alert(`请上传 Excel 文件（.xlsx/.xls）`);
        return
    }
    var n = document.getElementById(`import-inv-btn`),
        r = n ? n.innerHTML : ``;
    n && (n.disabled = !0, n.innerHTML = `<i class="fas fa-spinner fa-spin"></i> 导入中...`);
    try {
        var i = new FormData;
        i.append(`file`, t);
        var o = await Api.request(`POST`, `/tl/import_warehouse_inventory_excel`, i),
            s = _(o, [`msg`, `message`], ``) || _(Api.unwrapData(o), [`msg`, `message`], ``);
        alert(s || `导入成功`), invPg.page = 1, await loadInventoryList()
    } catch (e) {
        alert(`导入失败: ` + (e.message || String(e)))
    } finally {
        n && (n.disabled = !1, n.innerHTML = r || `<i class="fas fa-file-import"></i> 导入库存表格`)
    }
}

function rcpUpdatePager() {
    var e = document.getElementById(`rcp-page-info`),
        t = document.getElementById(`rcp-prev-btn`),
        n = document.getElementById(`rcp-next-btn`),
        r = Math.max(1, Math.ceil((rcpPg.total || 0) / rcpPg.pageSize));
    e && (e.textContent = `第 ` + rcpPg.page + ` / ` + r + ` 页（共 ` + rcpPg.total + ` 条）`), t && (t.disabled = rcpPg.page <= 1), n && (n.disabled = rcpPg.page >= r)
}
async function loadReceiptPriceList() {
    var e = document.getElementById(`rcp-table-body`),
        t = document.getElementById(`rcp-count`);
    if (!e) return;
    E(e, 4, `正在加载收货价格...`);
    try {
        var ws = document.getElementById(`rcp-wh-search`),
            cf = document.getElementById(`rcp-category-filter`),
            df = document.getElementById(`rcp-date-from`),
            dt = document.getElementById(`rcp-date-to`),
        ...
        rcpPg.total = a.total || 0, e.innerHTML = ``, o.length || D(e, 4, `暂无收货价格数据`), o.forEach(function(t) {
            var n = v(t, [`id`, `price_id`]),
                ...
                i = _(t, [`品类名`, `品类名称`, `category_name`], `-`),
                a = Number(_(t, [`价格`, `price`], null)),
                o = document.createElement(`tr`);
            var d = _(t, [`价格日期`, `price_date`, `日期`, `更新时间`, `updated_at`], `-`);
            if (d.length > 10) d = d.slice(0, 10);
            o.innerHTML =
                `<td>` + M(r) + `</td>` +
                `<td>` + M(i) + `</td>` +
                `<td>` + (isNaN(a) ? `-` : a.toLocaleString()) + `</td>` +
                `<td>` + M(d) + `</td>` +
                `<td>` +
                `<button class="btn btn-sm btn-outline edit-btn" data-id="${String(n)}">编辑</button> ` +
                `<button class="btn btn-sm btn-danger delete-btn" data-id="${String(n)}">删除</button>` +
                `</td>`;
            e.appendChild(o)
        }), t && (t.textContent = String(rcpPg.total)), rcpUpdatePager()
    } catch (n) {
        alert(`加载收货价格失败: ` + (n.message || String(n))), t && (t.textContent = `0`), e.innerHTML = ``, D(e, 4, n.message || `加载失败`), rcpUpdatePager()
    }
}
async function rcpManualSubmit() {
    var e = parseInt(document.getElementById(`rcp-warehouse`).value, 10),
        t = parseInt(document.getElementById(`rcp-category`).value, 10),
        n = parseFloat(document.getElementById(`rcp-price`).value);
    if (!e || isNaN(e)) throw Error(`请选择库房`);
    if (!t || isNaN(t)) throw Error(`请选择回收品种`);
    if (isNaN(n) || n < 0) throw Error(`请填写有效价格`);
    var rd = document.getElementById(`rcp-date`),
        rv = rd && rd.value ? rd.value.trim() : ``;
    var body = {
        库房id: e,
        品类id: t,
        价格: n
    ...
    if (rv) body.价格日期 = rv;
    await Api.request(`POST`, `/tl/warehouse_receipt_prices`, body), rcpPg.page = 1, await loadReceiptPriceList()
}
async function rcpEditPrice(e, t) {
    U(`编辑收货价格`, rcpEditFormHtml(), {
        onOpen: function() {
            var n = document.getElementById(`rcp-edit-price`);
            n && !isNaN(t) && (n.value = String(t));
            var ed = document.getElementById(`rcp-edit-date`);
            if (ed) {
                var dv = _(`t`, ["价格日期", "price_date", "日期"], "");
                ed.value = O(dv || "")
            }
        },
        onConfirm: async function() {
            var t = parseFloat(document.getElementById(`rcp-edit-price`).value);
            if (isNaN(t) || t < 0) throw Error(`请填写有效价格`);
            var ed = document.getElementById(`rcp-edit-date`),
                ev = ed && ed.value ? ed.value.trim() : ``;
            var body = {
                价格: t
            };
            if (ev) body.价格日期 = ev;
            await Api.request(`PUT`, `/tl/warehouse_receipt_prices/` + String(e), body), await loadReceiptPriceList()
        }
    })
}
async function rcpDeletePrice(e) {
    if (!e || isNaN(e)) return;
    if (!confirm(`确定删除该收货价格记录？`)) return;
    try {
        await Api.request(`DELETE`, `/tl/warehouse_receipt_prices/` + String(e)), await loadReceiptPriceList()
    } catch (e) {
        alert(`删除失败: ` + (e.message || String(e)))
    }
}
async function rcpDownloadTemplate() {
    try {
        var e = await Api.getBlob(`/tl/download_warehouse_receipt_prices_template_excel`),
            t = window.URL.createObjectURL(e),
            n = document.createElement(`a`);
        n.href = t, n.download = `库房收货价格导入模板.xlsx`, document.body.appendChild(n), n.click(), n.remove(), window.URL.revokeObjectURL(t)
    } catch (e) {
        alert(`下载模板失败: ` + (e.message || String(e)))
    }
}
async function rcpImportExcel(e) {
    var t = e && e[0];
    if (!t) return;
    if (!/\.(xlsx|xls)$/i.test(String(t.name || ``))) {
        alert(`请上传 Excel 文件（.xlsx/.xls）`);
    ...
    var n = document.getElementById(`import-rcp-btn`),
        r = n ? n.innerHTML : ``;
    n && (n.disabled = !0, n.innerHTML = `<i class="fas fa-spinner fa-spin"></i> 导入中...`);
    try {
        var i = new FormData;
        i.append(`file`, t);
        var a = await Api.request(`POST`, `/tl/import_warehouse_receipt_prices_excel`, i),
            o = _(a, [`msg`, `message`], ``) || _(Api.unwrapData(a), [`msg`, `message`], ``);
        alert(o || `导入成功`), rcpPg.page = 1, await loadReceiptPriceList()
    } catch (e) {
        alert(`导入失败: ` + (e.message || String(e)))
    } finally {
        n && (n.disabled = !1, n.innerHTML = r || `<i class="fas fa-file-import"></i> 导入价格表格`)
    }
}
async function initReceiptPricePage() {
    console.log(`初始化收货价格管理页面`);

    function _addRcpDateFilters() {
        var _rs = document.getElementById(`rcp-reset-btn`);
        if (!_rs) return false;
        var _rf = _rs.parentElement;
        if (!_rf) return false;
        if (!document.getElementById(`rcp-date-from`)) {
            var _df = document.createElement(`input`);
            _df.type = `date`;
            _df.id = `rcp-date-from`;
            _df.className = `form-control`;
            _df.title = `更新日期起`;
            _df.style.cssText = `max-width:150px;height:34px;`;
            _rf.insertBefore(_df, _rs)
        }
        if (!document.getElementById(`rcp-date-to`)) {
            var _dt = document.createElement(`input`);
            _dt.type = `date`;
            _dt.id = `rcp-date-to`;
            _dt.className = `form-control`;
            _dt.title = `更新日期止`;
            _dt.style.cssText = `max-width:150px;height:34px;`;
            _rf.insertBefore(_dt, _rs)
        }
        return true
    }
    if (!_addRcpDateFilters()) {
        var _rcpRetry = 0;
        var _rcpTimer = setInterval(function() {
            if (_addRcpDateFilters() || ++_rcpRetry > 15) clearInterval(_rcpTimer)
        }, 200)
    }
    var e = document.getElementById(`add-rcp-btn`);
    e && e.addEventListener(`click`, function() {
        U(`手工录入收货价格`, rcpFormHtml(), {
            onOpen: async function() {
                await setupModalWhCombo(`rcp`), await invFillCategorySelect(`rcp-category`);
                var rd = document.getElementById(`rcp-date`);
                rd && (rd.value = kt())
            },
            onConfirm: function() {
                return rcpManualSubmit()
            }
        })
    });
    var t = document.getElementById(`download-rcp-template-btn`);
    t && t.addEventListener(`click`, function() {
        rcpDownloadTemplate()
    });
    var n = document.getElementById(`import-rcp-btn`),
        r = document.getElementById(`rcp-import-input`);
    n && r && (n.addEventListener(`click`, function() {
        r.click()
    }), r.addEventListener(`change`, function(e) {
        rcpImportExcel(e && e.target ? e.target.files : null), r.value = ``
    }));
    var i = document.getElementById(`rcp-table-body`);
    i && i.addEventListener(`click`, function(e) {
        var t = e.target.closest(`.edit-btn`),
            n = e.target.closest(`.delete-btn`);
        if (t) {
            var r = parseInt(t.getAttribute(`data-id`), 10),
                i = t.closest(`tr`),
                a = i && i.children && i.children[2] ? parseFloat(String(i.children[2].textContent || ``).replace(/,/g, ``)) : NaN;
            rcpEditPrice(r, a)
        }
        n && rcpDeletePrice(parseInt(n.getAttribute(`data-id`), 10))
    });
    var a = document.getElementById(`rcp-search-btn`);
    a && a.addEventListener(`click`, function() {
        rcpPg.page = 1, loadReceiptPriceList()
    });
    var o = document.getElementById(`rcp-reset-btn`);
    o && o.addEventListener(`click`, function() {
        var wf = document.getElementById(`rcp-warehouse-filter`);
        wf && (wf.value = ``);
        var ws = document.getElementById(`rcp-wh-search`);
        ws && (ws.value = ``);
        var cf = document.getElementById(`rcp-category-filter`);
        cf && (cf.value = ``);
        var df = document.getElementById(`rcp-date-from`);
        df && (df.value = ``);
        var dt = document.getElementById(`rcp-date-to`);
        dt && (dt.value = ``);
        rcpPg.page = 1, loadReceiptPriceList()
    });
    var s = document.getElementById(`rcp-prev-btn`);
    s && s.addEventListener(`click`, function() {
        rcpPg.page > 1 && (--rcpPg.page, loadReceiptPriceList())
    });
    var c = document.getElementById(`rcp-next-btn`);
    c && c.addEventListener(`click`, function() {
        rcpPg.page += 1, loadReceiptPriceList()
    });
    var qRcpW = [];

    function _rcpWhApply() {
        renderWhComboList(`rcp-wh`, qRcpW, {
            allowAll: !0,
            allLabel: `全部库房`
        })
    }
    async function _initRcpFilters() {
        try {
            var ws = await Api.request(`GET`, `/tl/get_warehouses`);
            qRcpW = Api.unwrapList(ws).filter(function(x) {
                return !b(T(x))
            });
            var searchEl = document.getElementById(`rcp-wh-search`),
                listEl = document.getElementById(`rcp-wh-list`);
            if (searchEl) {
                searchEl.value = ``;
                ...
                document.addEventListener(`click`, function(e) {
                    if (!searchEl.contains(e.target) && listEl && !listEl.contains(e.target)) listEl.style.display = `none`
                })
            }
            if (listEl) {
                listEl.addEventListener(`mousedown`, function(e) {
                    var btn = e.target.closest(`[data-wh-id]`);
                    if (btn) {
                        setTimeout(function() {
            ...
            var resp = await Api.request(`GET`, `/tl/get_category_mapping`);
            var rawList = V(Api.unwrapData(resp) || {}).list;
            var cats = ue(se(Array.isArray(rawList) ? rawList : []));
            var cSel = document.getElementById(`rcp-category-filter`);
            if (cSel) {
                cSel.innerHTML = `<option value="">全部品种</option>` + cats.map(function(c) {
            ...
            console.warn(`加载收货价格筛选选项失败`, e)
        }
    }
    _initRcpFilters(), loadReceiptPriceList()
}
...
async function initInventoryPage() {
    console.log(`初始化当前库存管理页面`);
    var _fr = document.querySelector(`.inv-rcp-filter-row`);
    if (_fr) {
        if (!document.getElementById(`inv-date-from`)) {
            var _df = document.createElement(`input`);
            _df.type = `date`;
            ...
            _df.title = `库存日期起`;
            _df.style.cssText = `max-width:150px;height:34px;`;
            _fr.insertBefore(_df, _fr.querySelector(`#inv-reset-btn`) || null)
        }
        if (!document.getElementById(`inv-date-to`)) {
            var _dt = document.createElement(`input`);
            _dt.type = `date`;
            _dt.id = `inv-date-to`;
            _dt.className = `form-control`;
            _dt.title = `库存日期止`;
            _dt.style.cssText = `max-width:150px;height:34px;`;
            _fr.insertBefore(_dt, _fr.querySelector(`#inv-reset-btn`) || null)
        }
    }
    var e = document.getElementById(`add-inv-btn`);
    e && e.addEventListener(`click`, function() {
        U(`手工录入库存`, invFormHtml(), {
            onOpen: async function() {
                await setupModalWhCombo(`inv`), await invFillCategorySelect(`inv-category`);
                var e = document.getElementById(`inv-date`);
                e && !e.value && (e.value = kt())
            },
            onConfirm: function() {
                return invManualSubmit()
            }
        })
    });
    var t = document.getElementById(`download-inv-template-btn`);
    t && t.addEventListener(`click`, function() {
        invDownloadTemplate()
    });
    var n = document.getElementById(`import-inv-btn`),
        r = document.getElementById(`inv-import-input`);
    n && r && (n.addEventListener(`click`, function() {
        r.click()
    }), r.addEventListener(`change`, function(e) {
        invImportExcel(e && e.target ? e.target.files : null), r.value = ``
    }));
    var i = document.getElementById(`inv-table-body`);
    i && i.addEventListener(`click`, function(e) {
        var t = e.target.closest(`.edit-inv-btn`),
            d = e.target.closest(`.delete-inv-btn`);
        if (t) {
            var n = parseInt(t.getAttribute(`data-id`), 10),
                cid = parseInt(t.getAttribute(`data-category-id`), 10) || 0,
                r = parseFloat(t.getAttribute(`data-stock`)) || 0,
                i = t.getAttribute(`data-date`) || ``;
            invEditStock(n, r, i, cid)
        }
        if (d) {
            var did = parseInt(d.getAttribute(`data-id`), 10),
                ddate = d.getAttribute(`data-date`) || ``;
            invDeleteStock(did, ddate)
        }
    });
    var a = document.getElementById(`inv-search-btn`);
    a && a.addEventListener(`click`, function() {
        invPg.page = 1, loadInventoryList()
    });
    var o = document.getElementById(`inv-reset-btn`);
    o && o.addEventListener(`click`, function() {
        var wf = document.getElementById(`inv-wh-warehouse`);
        wf && (wf.value = ``);
        var ws = document.getElementById(`inv-wh-search`);
        ws && (ws.value = ``);
        var cf = document.getElementById(`inv-category-filter`);
        cf && (cf.value = ``);
    ...
    var s = document.getElementById(`inv-prev-btn`);
    s && s.addEventListener(`click`, function() {
        invPg.page > 1 && (--invPg.page, loadInventoryList())
    });
    var c = document.getElementById(`inv-next-btn`);
    c && c.addEventListener(`click`, function() {
        invPg.page += 1, loadInventoryList()
    });
    var l = document.getElementById(`inv-page-size`);
    l && l.addEventListener(`change`, function() {
        invPg.pageSize = parseInt(this.value, 10) || 20, invPg.page = 1, loadInventoryList()
    });
    var qInvW = [];

    function _invWhApply() {
        renderWhComboList(`inv-wh`, qInvW, {
            allowAll: !0,
            allLabel: `全部库房`
        })
    }
    async function _initInvFilters() {
        try {
            var ws = await Api.request(`GET`, `/tl/get_warehouses`);
            qInvW = Api.unwrapList(ws).filter(function(x) {
                return !b(T(x))
            });
            var searchEl = document.getElementById(`inv-wh-search`),
                listEl = document.getElementById(`inv-wh-list`);
            if (searchEl) {
                searchEl.value = ``;
                searchEl.addEventListener(`input`, function() {
                    listEl && (listEl.style.display = ``);
            ...
            var rawList = V(Api.unwrapData(resp) || {}).list;
            var cats = ue(se(Array.isArray(rawList) ? rawList : []));
            var cSel = document.getElementById(`inv-category-filter`);
            if (cSel) {
                cSel.innerHTML = `<option value="">全部品种</option>` + cats.map(function(c) {
                    var id = c.id,
                        n = String(c.name || ``).trim();
